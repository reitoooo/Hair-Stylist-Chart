...
interface AIResponse {
  ...
  styleChange?: string;
  colorChange?: string;
  designTypeChange?: string;
  accentColorChange?: string;
  permChange?: string;
...
export function generateAIResponse(
  ...
  let designTypeChange: string | undefined;
  let accentColorChange: string | undefined;
  ...
  if (imageUrl) {
    ...
    colorChange = '#9575cd'; // 模擬的にアッシュグレーに変更
...
この「アッシュグレージュ」をベースにシミュレーションを更新しました。他にご希望はありますか？`;
    return { text, tags, styleChange, colorChange, designTypeChange, accentColorChange, permChange };
  }
...
他にも「前髪の長さ」や「サイドの量感」など、気になる部分はありますか？`;
    ...
    return { text, tags, styleChange, colorChange, designTypeChange, accentColorChange, permChange, followUpQuestion };
  }
...
お好みを教えていただければ、シミュレーションに反映します！`;
  ...
  // ─── Color Design Types ───
  if (matchesAny(textLower, ['ハイライト', 'メッシュ'])) {
    designTypeChange = 'highlight';
    tags.push('デザインカラー：ハイライト');
  } else if (matchesAny(textLower, ['インナーカラー', 'インナー', 'イヤリング'])) {
    designTypeChange = 'inner';
    tags.push('デザインカラー：インナーカラー');
  } else if (matchesAny(textLower, ['グラデーション', 'グラデ'])) {
    designTypeChange = 'gradation';
    tags.push('デザインカラー：グラデーション');
  } else if (matchesAny(textLower, ['バレイヤージュ'])) {
    designTypeChange = 'balayage';
    tags.push('デザインカラー：バレイヤージュ');
  } else if (matchesAny(textLower, ['単色', 'ワンカラー', '全体'])) {
    designTypeChange = 'solid';
  }
  ...
  if (matchesAny(textLower, ['アッシュ', 'グレー', '灰'])) {
    // #9575cd
    colorChange = '#9575cd'; matchedColorName = 'アッシュグレー'; colorChanged = true;
  } else if (matchesAny(textLower, ['ベージュ', 'ミルクティー'])) {
    // #d4b895 (using a similar hex from presets, e.g. #d7ccc8)
    colorChange = '#d7ccc8'; matchedColorName = 'ミルクティーベージュ'; colorChanged = true;
  } else if (matchesAny(textLower, ['ピンク', '赤'])) {
    // #f06292
    colorChange = '#f06292'; matchedColorName = 'ピンクブラウン'; colorChanged = true;
  } else if (matchesAny(textLower, ['ブラウン', '茶'])) {
    // #a1887f
    colorChange = '#a1887f'; matchedColorName = 'ナチュラルブラウン'; colorChanged = true;
  } else if (matchesAny(textLower, ['ブラック', '黒', '暗髪'])) {
    // #212121
    colorChange = '#212121'; matchedColorName = 'ブルーブラック'; colorChanged = true;
  }
